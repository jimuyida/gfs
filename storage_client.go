...
func (this *StorageClient) storageDownloadFile(tc *TrackerClient,
	...
	resp.FileId = storeServ.groupName + string(PathSeparator) + remoteFilename
 ...
 * remoteFilename: 远程文件名（不含组名称）
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
func (this *StorageClient) storageQueryFileInfo(tc *TrackerClient, storeServ *StorageServer, remoteFilename string)(*QueryFileInfoResponse,error) {
	var conn net.Conn
	var reqBuf []byte
	var err error
	...
	//发送删除文件包
	req := &queryFileInfoRequest{}
	req.groupName = storeServ.groupName
	req.remoteFilename = remoteFilename
	reqBuf, err = req.marshal()
	if err != nil {
		log.Printf("query file info marshal error :%s", err.Error())
		return nil,err
	}
	TcpSendData(conn, reqBuf)
	...
	recvBuff, recvSize, err := TcpRecvResponse(conn, th.pkgLen)
	if recvSize != 3*int64(FDFS_PROTO_PKG_LEN_SIZE)+IP_ADDRESS_SIZE {
		errmsg := "[-] Error: Storage response length is not match, "
		errmsg += fmt.Sprintf("expect: %d, actual: %d", th.pkgLen, recvSize)
		log.Print(errmsg)
		return nil,errors.New(errmsg)
...
func (this *StorageClient) storageModifyByFilename(tc *TrackerClient,
	...
	return this.storageDoModifyFile(tc, storeServ, filename, int64(fileSize), fileOffset, FDFS_UPLOAD_BY_FILENAME,
		remoteFileId)
}
...
func (this *StorageClient) storageModifyBuffer(tc *TrackerClient,
	...
	return this.storageDoModifyFile(tc, storeServ, fileBuffer, int64(bufferSize),fileOffset, FDFS_UPLOAD_BY_BUFFER,
		remoteFileId )
}
 ...
 * fileContent: 文件名 | 字节数组
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
func (this *StorageClient) storageDoModifyFile(tc *TrackerClient,
	storeServ *StorageServer, fileContent interface{}, fileSize int64, fileOffset int64,uploadType int,
	masterFilename string) (error) {
	...
	masterFilenameLen := int64(len(masterFilename))
	if len(storeServ.groupName) > 0 && len(masterFilename) > 0 {
		headerLen = int64(FDFS_PROTO_PKG_LEN_SIZE)*3 + masterFilenameLen
	}
	...
	if err != nil {
		log.Printf("modifyFileRequest.marshal error :%s", err.Error())
		return err
	}
