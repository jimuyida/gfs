...
type (
	...
	queryFileInfoRequest struct {
		groupName      string
		remoteFilename string
	}

	modifyFileRequest struct {
		masterFilenameLen int64
		fileOffset		  int64
		fileSize          int64
		masterFilename    string
...
**/
func (this *modifyFileRequest) marshal() ([]byte, error) {
	buffer := new(bytes.Buffer)
	binary.Write(buffer, binary.BigEndian, this.masterFilenameLen)
	binary.Write(buffer, binary.BigEndian, this.fileOffset)
	binary.Write(buffer, binary.BigEndian, this.fileSize)
	...
	//buffer.WriteString(this.masterFilename)
	return buffer.Bytes(), nil
}

// recv_fmt: |-group_name(16)-remote_file_name(recv_size - 16)-|
func (this *UploadResponse) unmarshal(data []byte) error {
	buff := bytes.NewBuffer(data)
	var err error
	...
	this.FileId = this.GroupName + string(PathSeparator) + remoteFilename
	return nil
}
...
func (this *queryFileInfoRequest) marshal() ([]byte, error) {
	buffer := new(bytes.Buffer)
...
// recv_fmt: |-group_name(16)-remote_file_name(recv_size - 16)-|
func (this *DeleteResponse) unmarshal(data []byte) error {
	buff := bytes.NewBuffer(data)
	var err error
	this.groupName, err = readCstr(buff, FDFS_GROUP_NAME_MAX_LEN)
	if err != nil {
...
func (this *QueryFileInfoResponse) unmarshal(data []byte) error {
	buff := bytes.NewBuffer(data)
	this.fileSize = buff2long(buff,data)
	this.createTimestamp = buff2long(buff,data)
	this.crc32 = buff2long(buff,data)
	this.ipAddr = buff.String()
	return nil
}
